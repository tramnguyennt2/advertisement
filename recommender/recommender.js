...
module.exports = {
  getContentBasedResult: function(item_id) {
    return new Promise(function(resolve, reject) {
      handle_file.readDocsFile(cbJsonFile).then(cbs => {
        resolve(cbs[item_id]);
      });
    });
...
function joinAndReturn(user_id, item_id) {
  ...
  doAlgorithms(user_id, item_id).then(results => {
    let cb_results = results[0],
      cf_results = results[1];
    cb_results.forEach(cb_item => {
      if (unique_arr.indexOf(cb_item.id) <= -1) unique_arr.push(cb_item.id);
    });
    cf_results.forEach(cf_item => {
      if (unique_arr.indexOf(cf_item.id) <= -1) unique_arr.push(cf_item.id);
    });
    ...
    sort(output).then(r => {
      if (r.length > 10) result = r.splice(0, 10);
      else result = r;
    });
    done = true;
  });
